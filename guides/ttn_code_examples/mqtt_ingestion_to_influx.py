...
###################################################################################################################################
#### Check all settings, name choice etc - make sure you understand what you copypaste! some of it HAS TO BE MODIFIED/PERSONALIZED!
###################################################################################################################################
...
measurement_name = "<userspecific_name>"
...
def on_ttn_message(client, userdata, msg):
    ...
    if "decoded_payload" in message["uplink_message"].keys():
        ...
        decoded_pressure = message["uplink_message"]["decoded_payload"].get("pressure")
        decoded_voc = message["uplink_message"]["decoded_payload"].get("voc")
        decoded_altitude = message["uplink_message"]["decoded_payload"].get("altitude")
        print(decoded_co2, decoded_humidity, decoded_temperature, decoded_pressure, decoded_voc, decoded_altitude)
        # we receive and read the relevant information into tags and fields
